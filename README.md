...
<img src="packaging/netvault.png" width="64" alt="NetVault"> <img src="packaging/netmaster.png" width="64" alt="NetMaster">
...
Собранные `NetVault.exe` и `NetMaster.exe` можно скопировать на любой компьютер
с Windows — Python и библиотеки там не нужны.

**Скачать готовую сборку.** На GitHub: вкладка **Actions** → слева
«Сборка программ для Windows» → **Run workflow** → дождаться зелёной галочки →
внизу страницы запуска скачать архив `NetVault-NetMaster-windows`. Внутри оба файла.

**Собрать самому** (нужен Python с галочкой «tcl/tk and IDLE»):

```bash
pip install -r requirements.txt
python build.py                 # обе программы, каждая одним файлом
