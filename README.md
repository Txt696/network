# NetVault + NetMaster + NetMap

<img src="packaging/netvault.png" width="64" alt="NetVault"> <img src="packaging/netmaster.png" width="64" alt="NetMaster"> <img src="packaging/netmap.png" width="64" alt="NetMap">

Три программы для сетевого инженера, работающие с одним хранилищем данных.
...
#### Целая папка конфигов сразу

Когда конфигов не один, а сколько есть железа, по одному их открывать незачем:
**«Импорт папки…»** (или «Устройство → Импортировать папку с конфигами…»)
берёт папку целиком — все `.txt`, `.cfg`, `.conf`, `.log`, `.ios`, `.run`,
`.rsc`, включая подпапки, — и записывает устройства в хранилище сразу, без
захода в форму на каждое. То же самое из консоли:

```bash
python netvault/cli.py import D:\configs                # папка целиком
python netvault/cli.py import sw1.txt sw2.txt --site DC1  # отдельные файлы
python netvault/cli.py import D:\configs --flat          # без подпапок
```

Пачка разбирается в два прохода: сначала NetVault узнаёт, какие устройства в
ней вообще есть, и только потом разбирает описания портов. Поэтому связи
внутри пачки получаются заполненными с обеих сторон независимо от того, в
каком порядке лежат файлы: `description To SB-AS-Master-rt` на свитче и
`description To BFN-AS-ME` на роутере сходятся в один линк `Gi1/0/1 — Po1`.
Описание порта читается терпимо: `To core-sw-01`, `For SB-AS-Master-rt`,
`To BFN-AS-ME (Port-channel1)`, `To SB-AS-Master-rt Cross-1`,
`To TMCell (Vl40-SMS-Gate, Vl135-APN)` — имя соседа берётся, хвост в скобках,
после запятой и номер кросса отбрасываются.

Сокращённое имя узнаётся по вхождению. Если под него подходит несколько
устройств (`To BFN-AS-ME` при живых BFN-AS-ME-SB-sw и BFN-AS-ME-SB-tkm-sw),
спрашивается встречная сторона: сосед тот, у кого в описаниях портов упомянуты
мы. Не разошлось и так — связь остаётся по имени, гадать NetVault не будет.

Порт с `channel-group N` связь себе не забирает, если она уже описана на
`Port-channel N`: физических кабелей столько, сколько членов агрегата, а линк
один — на карте вместо трёх линий к одному соседу рисуется одна, Po1 — Po1.
Один порт соседа тоже достаётся ровно одному линку.

Устройство с таким же именем обновляется, а не задваивается: порты, VLAN, IP
и аплинки берутся из конфига, а площадка, теги, стойка, статус и доступы
остаются как были — конфиг о них ничего не знает. Аплинки портов, которых в
конфиге не было (проставленные руками или пришедшие обратной синхронизацией
от соседа), тоже остаются: конфиг их не отменяет, он про них просто молчит.
Импортировать одну и ту же папку повторно безопасно — это обновление, а не
новые записи. Файл, в котором нет команды `hostname`, пропускается с
пояснением в отчёте, остальные при этом импортируются.

`--site` проставляется только новым устройствам. По итогам показывается
отчёт: что добавлено, что обновлено, сколько портов, VLAN и аплинков вышло
и на что стоит посмотреть руками.
...
Собранные `NetVault.exe`, `NetMaster.exe` и `NetMap.exe` можно скопировать
...
python build.py                 # все три программы, каждая одним файлом
...
Ожидаемый вывод: `Ran 127 tests ... OK`.
